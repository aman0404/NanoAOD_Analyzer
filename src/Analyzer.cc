...
Analyzer::Analyzer(std::vector<std::string> infiles, std::string outfile, bool setCR, std::string configFolder, std::string year) : goodParts(getArray()), genName_regex(".*([A-Z][^[:space:]]+)"){
  ...
  */
  ...
  initializeShapeCorrection(infiles);
  initializeTauFakeFactor(infiles);
...
void Analyzer::fill_histogram(std::string year) {
  ...
  if(!isData){
    ...
    if(distats["Run"].bfind("ApplyTauFakeRate")) wgt *= TauFakeRate(); 
    if(distats["Run"].bfind("ApplyShapeCorrection")) wgt*=ShapeCorrection();
	}
...
void Analyzer::fill_Folder(std::string group, const int max, Histogramer &ihisto, bool issyst) {
  ...
  if(group == "FillRun" && (&ihisto==&histo)) {
    ...
    histAddVal2(part->pt(ptIndexVector.back().second),part->eta(ptIndexVector.back().second), "FirstLeadingPtVsFirstLeadingEta");
//Aman
	}
    ...
    */
    ...
    for(auto it : *active_part->at(ePos)) {
      ...
      histAddVal(cos(absnormPhi(atan2(part1.Py() - part2.Py(), part1.Px() - part2.Px()) - _MET->phi())), "CosDphi_DeltaPtAndMet");
//Aman
      histAddVal2(absnormPhi(part1.Phi() - _MET->phi()),_MET->pt(),"Part1MetDeltaPhiVsMet");
      histAddVal2(absnormPhi(part2.Phi() - _MET->phi()),_MET->pt(),"Part2MetDeltaPhiVsMet");
...
//Aman edits
      TLorentzVector lepSum(0,0,0,0);
      lepSum = part1 + part2;

      histAddVal(absnormPhi(lepSum.Phi() - _MET->phi()), "DiMetDeltaPhi");
//Aman edits end
...
void Analyzer::initializeTauFakeFactor(std::vector<std::string> infiles){

 TFile hist_fakeTau("Pileup/DeeptauFake_MC_2018.root");
 Taufake_h =dynamic_cast<TH1D*>(hist_fakeTau.FindObjectAny("h5"));
 //fake_h =dynamic_cast<TH2F*>(hist_fakeTau.FindObjectAny("h1"));
 hist_fakeTau.Close();

}

void Analyzer::initializeShapeCorrection(std::vector<std::string> infiles){

	TFile hist_shapetau("Pileup/tau_shape_corr_mutau_2018.root");
	shapeTau_h = dynamic_cast<TH1D*>(hist_shapetau.FindObjectAny("hdiv1"));
	hist_shapetau.Close();
	
	TFile hist_shapemu("Pileup/mu_shape_corr_mutau_2018.root");
//	TFile hist_shapemu("Pileup/elec_shape_corr_etau_2018.root");
	shapeMu_h = dynamic_cast<TH1D*>(hist_shapemu.FindObjectAny("hdiv1"));
	hist_shapemu.Close();
	}
...
//2D
/* 
double Analyzer::TauFakeRate(){
        double temp = 1.;
        double fakerate = 1.;
        if(active_part->at(CUTS::eRTau1)->size()&&active_part->at(CUTS::eRMuon1)->size()){
        double tauPt = _Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Pt();
        double tauEta = _Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Eta();
                                int binx = fake_h->GetXaxis()->FindBin(tauPt);
                                int biny = fake_h->GetYaxis()->FindBin(tauEta);
                                temp = fake_h->GetBinContent(binx,biny);        //original
                                fakerate = temp/(1-temp);
                                }
                                return fakerate;
                                }
*/
...
double Analyzer::TauFakeRate(){
        double  Taufakerate = 1.;
                double temp = 1.;

//                if(active_part->at(CUTS::eRElec1)->size()&&active_part->at(CUTS::eRTau1)->size()){
               if(active_part->at(CUTS::eRMuon1)->size()&&active_part->at(CUTS::eRTau1)->size()){
		double tauPt = _Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Pt();
                int binx = Taufake_h->GetXaxis()->FindBin(tauPt);
                temp = Taufake_h->GetBinContent(binx);

		Taufakerate = temp/(1-temp);
                }
                return Taufakerate;
                        }
...
//Aman
double Analyzer::ShapeCorrection(){
	...
	if(active_part->at(CUTS::eRMuon1)->size()&&active_part->at(CUTS::eRTau1)->size()){
//	if(active_part->at(CUTS::eRElec1)->size()&&active_part->at(CUTS::eRTau1)->size()){
	double tauPhi = _Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Phi();
	double metPhi = _MET->phi();
	double muPhi  = _Muon->p4(active_part->at(CUTS::eRMuon1)->at(0)).Phi();
//	double ePhi = _Electron->p4(active_part->at(CUTS::eRElec1)->at(0)).Phi();	

	double tau_delphi = absnormPhi(tauPhi-metPhi);
	double mu_delphi = absnormPhi(muPhi-metPhi);
//	double e_delphi = absnormPhi(ePhi - metPhi);

	int bin_tau = shapeTau_h->GetXaxis()->FindBin(tau_delphi);
	int bin_mu = shapeMu_h->GetXaxis()->FindBin(mu_delphi);
//	int bin_e = shapeMu_h->GetXaxis()->FindBin(e_delphi);
	
	temp_tau = shapeTau_h->GetBinContent(bin_tau);
	temp_mu = shapeMu_h->GetBinContent(bin_mu);
//	temp_e  =  shapeMu_h->GetBinContent(bin_e);
//	temp_mu = shapeMu_h->GetBinContent(bin_mu);

//	correction_factor = temp_tau*temp_e;
	correction_factor = temp_tau*temp_mu;
//	correction_factor = temp_tau;
//	std::cout<<" factor "<< temp_tau<<'\t'<<temp_mu<<'\t'<<correction_factor<<std::endl;
	
	//std::cout<<" check this "<<e_delphi<<'\t'<<temp_mu<<std::endl;
